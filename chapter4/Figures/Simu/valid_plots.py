...
plt.figure()
plt.plot(times[1:],(tx[1:]), label="Simulation",linestyle="", marker='x', color=colors[0],markevery=markevery)
# ~ plt.plot(time,txold, label="x int",linestyle="", marker='x', color=colors[0],markevery=markevery)

plt.plot(times[1:], (xi)*times[1:]**0.5, color=colors[0], label="Analytical solution")
...
#plot speeds
plt.figure()
plt.plot(times[1:],tv, label="v01",linestyle="", marker='x', color=colors[0],markevery=markevery)

plt.plot(times[1:], (xi)/2/times[1:]**0.5, color=colors[0])

plt.legend()
plt.title("Interface speed, L1 relative error is "+("%5.2f" %errp_v)+"$\%$" )
plt.xscale("log")
...
cutoff=60
imax=500
#  cutoff=60 # for metastable
#  imax=1500
speed_oscillation=tv-(xi)/2/times[1:]**0.5
sig=speed_oscillation[cutoff:imax]
plt.plot(sig, label="v oscillation",linestyle="-", marker='x',markevery=markevery)
x = np.arange(len(sig))
#  u,l=envelope(x,speed_oscillation[cutoff:])

#  for i in range(2):
    #  u,_=envelope(x,u(x))
#  for i in range(2):
    #  _,l=envelope(x,l(x))
u,l=envelope_monotonous(x,sig,uvar="-",lvar="+")
plt.plot(u(x))
plt.plot(l(x))
#  plt.xscale('log')
#  plt.yscale('symlog',linthresh=0.01)
plt.legend()
plt.title("Speed oscillation")
#  plt.figure()
imax=-3
#  plt.plot(np.log(x[1:imax]),np.log(u(x[1:imax])))
pp,res=np.polynomial.Polynomial.fit(np.log((x[1:imax]+cutoff)*dt*noutput),np.log(u(x[1:imax])),1, full = True)
plt.plot(np.exp(pp(np.log((x[1:imax]+cutoff)*dt*noutput))))
print(res[0])
print(pp)
print("envelope = %f x t^%f" % (np.exp(pp.coef[0]),pp.coef[1]))
pp,res=np.polynomial.Polynomial.fit((x[1:imax]+cutoff)*dt*noutput,np.log(u(x[1:imax])),1, full = True)
plt.plot(np.exp(pp((x[1:imax]+cutoff)*dt*noutput)))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot1 = ax_diag.plot(sol1.tC1, sol1.tC2, linewidth=2, linestyle="--", label=("$\\xi$=% 5.3f" % (sol1.xi)))
...
ax_diag.plot(tC1,tC2,linewidth=0,marker="o",markevery=markevery,zorder=-1, label="Simulation at %5.3f" % (times[timestep]))
