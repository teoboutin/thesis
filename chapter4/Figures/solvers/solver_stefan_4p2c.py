...
class Stefan4p2cSolution:
    ...
    def __init__(self,Thermo, Diff, Init,sol):
        ...
        xi12,xi23,xi34,zeta12,zeta23,zeta34=sol.x
        ...
        if xi34==xi23:
            ...
            #  CompCoeffs["B3A"]=(CompCoeffs["C323A"]-CompCoeffs["C334A"])/(special.erfc(CompCoeffs["xi34"]/2/D3_1**0.5)-special.erfc(CompCoeffs["xi23"]/2/D3_1**0.5))
        CompCoeffs["A3A"]=CompCoeffs["C334A"]-CompCoeffs["B3A"]*special.erfc(-xi34/2/D3_1**0.5)
        CompCoeffs["A3A2"]=CompCoeffs["C323A"]-CompCoeffs["B3A"]*special.erfc(-xi23/2/D3_1**0.5)
        
        if xi34==xi23:
            CompCoeffs["B3B"]=0
        else:
            CompCoeffs["B3B"]=(CompCoeffs["C323B"]-CompCoeffs["C334B"])/(special.erfc(-xi23/2/D3_2**0.5)-special.erfc(-xi34/2/D3_2**0.5))
            #  CompCoeffs["B3B"]=(CompCoeffs["C323B"]-CompCoeffs["C334B"])/(special.erfc(xi34/2/D3_2**0.5)-special.erfc(xi23/2/D3_2**0.5))
        CompCoeffs["A3B"]=CompCoeffs["C334B"]-CompCoeffs["B3B"]*special.erfc(-xi34/2/D3_2**0.5)
        CompCoeffs["A3B2"]=CompCoeffs["C323B"]-CompCoeffs["B3B"]*special.erfc(-xi23/2/D3_2**0.5)
    
        # fourth phase
        CompCoeffs["A4A"]=Init.Clinf1
        CompCoeffs["B4A"]=(CompCoeffs["C434A"]-Init.Clinf1)/special.erfc(xi34/2/D4_1**0.5)
        CompCoeffs["A4B"]=Init.Clinf2
        CompCoeffs["B4B"]=(CompCoeffs["C434B"]-Init.Clinf2)/special.erfc(xi34/2/D4_2**0.5)
...
class SolverStefan4p2c:
    ...
    def u112A(self,x12):
        ...
        return (self.D1_1/np.pi)**0.5*np.exp(-l112**2)/special.erfc(-l112)
        
    def u112B(self,x12):
        l112=x12/2/(self.D1_2)**0.5
        return (self.D1_2/np.pi)**0.5*np.exp(-l112**2)/special.erfc(-l112)
        
    def u212A(self,x12,x23):
        D=self.D2_1
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return (D/np.pi)**0.5*np.exp(-l212**2)/(1+self.use_divide*(self.du212A(x12,x23)-1))
        
    def du212A(self,x12,x23):
        D=self.D2_1
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return (special.erfc(l223)-special.erfc(l212))
        
    def u212B(self,x12,x23):
        D=self.D2_2
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return (D/np.pi)**0.5*np.exp(-l212**2)/(1+self.use_divide*(self.du212B(x12,x23)-1))
        
    def du212B(self,x12,x23):
        D=self.D2_2
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return (special.erfc(l223)-special.erfc(l212))
    ...
    def u223A(self,x12,x23):
        ...
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return -(D/np.pi)**0.5*np.exp(-l223**2)/(1+self.use_divide*(self.du223A(x12,x23)-1))
        
    def du223A(self,x12,x23):
        D=self.D2_1
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return (special.erfc(l223)-special.erfc(l212))
    ...
    def u223B(self,x12,x23):
        ...
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return -(D/np.pi)**0.5*np.exp(-l223**2)/(1+self.use_divide*(self.du223B(x12,x23)-1))
        
    def du223B(self,x12,x23):
        D=self.D2_2
        l212=x12/2/(D)**0.5
        l223=x23/2/(D)**0.5
        return (special.erfc(l223)-special.erfc(l212))
    ...
    def u323A(self,x23,x34):
        ...
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return -(D/np.pi)**0.5*np.exp(-l323**2)/(1+self.use_divide*(self.du323A(x23,x34)-1))
        
    def du323A(self,x23,x34):
        D=self.D3_1
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return (special.erfc(-l323)-special.erfc(-l334))
        
    def u323B(self,x23,x34):
        D=self.D3_2
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return -(D/np.pi)**0.5*np.exp(-l323**2)/(1+self.use_divide*(self.du323B(x23,x34)-1))
        
    def du323B(self,x23,x34):
        D=self.D3_2
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return (special.erfc(-l323)-special.erfc(-l334))
    ...
    def u334A(self,x23,x34):
        ...
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return (D/np.pi)**0.5*np.exp(-l334**2)/(1+self.use_divide*(self.du334A(x23,x34)-1))
        
    def du334A(self,x23,x34):
        D=self.D3_1
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return (special.erfc(-l323)-special.erfc(-l334))
    ...
    def u334B(self,x23,x34):
        ...
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return (D/np.pi)**0.5*np.exp(-l334**2)/(1+self.use_divide*(self.du334B(x23,x34)-1))
        
    def du334B(self,x23,x34):
        D=self.D3_2
        l323=x23/2/(D)**0.5
        l334=x34/2/(D)**0.5
        return (special.erfc(-l323)-special.erfc(-l334))
    ...
    def u434A(self,x34):
        D=self.D4_1
        l434=x34/2/(D)**0.5
        return (D/np.pi)**0.5*np.exp(-l434**2)/(special.erfc(l434))
    ...
    def f1(self,x12,zeta12,x23,zeta23):
        lhs  = x12/2*(1-zeta12)*(self.Thermo.Cl0_1-self.Thermo.Cs0_1) 
        lhs *= (1+(1-self.use_divide)*(self.du212A(x12,x23)-1))
        rhs  = self.u112A(x12)*((1-zeta12)*self.Thermo.Cs0_1-self.Init.Csinf1) * (1+(1-self.use_divide)*(self.du212A(x12,x23)-1)) 
        rhs += self.u212A(x12,x23)*(zeta12-zeta23)*self.Thermo.Cl0_1
        return rhs-lhs
        
    def f2(self,x12,zeta12,x23,zeta23,x34,zeta34):
        lhs  = x23/2*(1-zeta23)*(self.Thermo.Cs0_1-self.Thermo.Cl0_1) 
        lhs *= (1+(1-self.use_divide)*(self.du223A(x12,x23)-1)) * (1+(1-self.use_divide)*(self.du323A(x23,x34)-1))
        rhs  = self.u223A(x12,x23)*(zeta12-zeta23)*self.Thermo.Cl0_1 * (1+(1-self.use_divide)*(self.du323A(x23,x34)-1)) 
        rhs += self.u323A(x23,x34)*(zeta34-zeta23)*self.Thermo.Cs0_1 * (1+(1-self.use_divide)*(self.du223A(x12,x23)-1))
        return rhs-lhs
        
    def f3(self,x23,zeta23,x34,zeta34):
        lhs  = x34/2*(1-zeta34)*(self.Thermo.Cl0_1-self.Thermo.Cs0_1) * (1+(1-self.use_divide)*(self.du334A(x23,x34)-1))
        rhs  = self.u334A(x23,x34)*(zeta34-zeta23)*self.Thermo.Cs0_1 
        rhs += self.u434A(x34)*((1-zeta34)*self.Thermo.Cl0_1-self.Init.Clinf1) * (1+(1-self.use_divide)*(self.du334A(x23,x34)-1))
        return rhs-lhs
        
    def f4(self,x12,zeta12,x23,zeta23):
        lhs=x12/2*(zeta12)*(self.Thermo.Cl0_2-self.Thermo.Cs0_2) * (1+(1-self.use_divide)*(self.du212B(x12,x23)-1))
        rhs  = self.u112B(x12)*((zeta12)*self.Thermo.Cs0_2-self.Init.Csinf2)  * (1+(1-self.use_divide)*(self.du212B(x12,x23)-1))
        rhs += self.u212B(x12,x23)*(zeta23-zeta12)*self.Thermo.Cl0_2
        return rhs-lhs
        
    def f5(self,x12,zeta12,x23,zeta23,x34,zeta34):
        lhs  = x23/2*(zeta23)*(self.Thermo.Cs0_2-self.Thermo.Cl0_2) 
        lhs *= (1+(1-self.use_divide)*(self.du223B(x12,x23)-1)) * (1+(1-self.use_divide)*(self.du323B(x23,x34)-1))
        rhs  = self.u223B(x12,x23)*(zeta23-zeta12)*self.Thermo.Cl0_2 * (1+(1-self.use_divide)*(self.du323B(x23,x34)-1))
        rhs += self.u323B(x23,x34)*(zeta23-zeta34)*self.Thermo.Cs0_2 * (1+(1-self.use_divide)*(self.du223B(x12,x23)-1)) 
        return rhs-lhs
        
    def f6(self,x23,zeta23,x34,zeta34):
        lhs  = x34/2*(zeta34)*(self.Thermo.Cl0_2-self.Thermo.Cs0_2)
        lhs *= (1+(1-self.use_divide)*(self.du334B(x23,x34)-1))
        rhs  = self.u334B(x23,x34)*(zeta23-zeta34)*self.Thermo.Cs0_2
        rhs += self.u434B(x34)*((zeta34)*self.Thermo.Cl0_2-self.Init.Clinf2) * (1+(1-self.use_divide)*(self.du334B(x23,x34)-1))
        return rhs-lhs
        
    # transcendental function to solve for 0
    def f(self,vect):
        x12,x23,x34,zeta12,zeta23,zeta34=vect
        
        res1=self.f1(x12,zeta12,x23,zeta23)
        res2=self.f2(x12,zeta12,x23,zeta23,x34,zeta34)
        res3=self.f3(x23,zeta23,x34,zeta34)
        res4=self.f4(x12,zeta12,x23,zeta23)
        res5=self.f5(x12,zeta12,x23,zeta23,x34,zeta34)
        res6=self.f6(x23,zeta23,x34,zeta34)
    ...
    def solve(self,start, maxfev):
        ...
        return Stefan4p2cSolution(self.Thermo, self.Diff, self.Init, sol)
...
# input parameters
    ...
    Solver = SolverStefan4p2c(Thermo, Diff, Init)
    ...
    ax_diag.plot(sol1.tC1, sol1.tC2, linewidth=2, linestyle="--")
