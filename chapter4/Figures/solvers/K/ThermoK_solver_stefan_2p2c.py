...
# print the params
    ...
    sol1 = Solver.solve([0.1,0,0], 1000)

    fig_diag, ax_diag = plt.subplots()
    
    Thermo.compute_diagram_from_c()
    
    plt.plot([0,1,0,0], [0,0,1,0], color="k")
    
    Na2O=1
    ax_diag.scatter(Thermo.front0[Na2O], Thermo.front0[1-Na2O], color="b", label="Frontiere phase 0")
    ax_diag.scatter(Thermo.front1[Na2O], Thermo.front1[1-Na2O], color="r", label="Frontiere phase 1")
    
    
    ax_diag.set_xlabel("Na2O")
    ax_diag.set_ylabel("SiO2")
    # ~ ax_diag.scatter([ciniA], [ciniB], color="purple", label="Cini")
    ...
    ax_diag.scatter([Init.Csinf2, Init.Clinf2], [Init.Csinf1,Init.Clinf1], color="k",zorder=5)

    ax_diag.set_yticks([tick for tick in ax_diag.get_yticks() if int(tick) % 5 == 0])

    
    p1color = ax_diag.plot(sol1.tC2, sol1.tC1, linewidth=2, linestyle="--", label=("$\\xi$=%f" % (sol1.xi)))[0].get_color()
